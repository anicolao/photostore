<script lang="ts">
  ...
  onMount(async () => {
    try {
      response = await getUndatedPhotos();
    } catch (err) {
      error = String(err);
    } finally {
      loading = false;
...
<svelte:head>
  <title>Photostore undated photos</title>
</svelte:head>
    ...
    <section>
      <div class="photo-grid" data-testid="undated-photo-grid">
        {#each response?.photos ?? [] as photo}
          <a class="photo-card" data-testid="undated-photo-card" href={photo.view_url} target="_blank" rel="noreferrer">
