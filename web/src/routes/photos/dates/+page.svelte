<script lang="ts">
  ...
  import { getPhotoYears } from '$lib/api';
...
<svelte:head>
  <title>Photostore photos by date</title>
</svelte:head>
  ...
  {:else}
    <section>
      {#if response?.buckets.length === 0}
        <p class="empty" data-testid="date-empty">No dated photos yet.</p>
      {:else}
        <div class="bucket-grid" data-testid="date-bucket-grid">
          {#each response?.buckets ?? [] as bucket}
            <a class="bucket-card" data-testid="date-bucket" href={`/photos/dates/${bucket.bucket_key}`}>
      ...
      {/if}
      <a class="undated" data-testid="undated-link" href="/photos/undated">Undated photos</a>
    </section>
  {/if}
</main>
