<script lang="ts">
  ...
  import { getPhotoDays } from '$lib/api';
  ...
  onMount(async () => {
    ...
    try {
      response = await getPhotoDays(year, month);
    } catch (err) {
      error = String(err);
    } finally {
      loading = false;
...
<svelte:head>
  <title>Photostore photos {year}-{month}</title>
</svelte:head>

<main>
  <header>
    <div>
      <a href={`/photos/dates/${year}`}>Year</a>
      <h1>{year}-{month}</h1>
    </div>
  </header>
    ...
    <section>
      <div class="bucket-grid" data-testid="date-bucket-grid">
        {#each response?.buckets ?? [] as bucket}
          <a class="bucket-card" data-testid="date-bucket" href={`/photos/dates/${bucket.bucket_key.replaceAll('-', '/')}`}>
